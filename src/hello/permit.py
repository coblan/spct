...
permit1 = [
        ...
        {'label': '报表中心', 
         'children': [
            ...
            {'label':'活动统计','value':'report.reportBonus'}
                     ],
        }, 
        {'label': '代理平台', 'children': [
        ...
        {
            'label':'三方平台','visible':getattr(settings,'OPEN_SECRET',False),'children':[
                {'label':'AG系统','children':[
                {'label':'AG账号','children':[
...
def get_permit(ui=None): 
    ...
    if not ui:
        ...
        out_permit = permit_merchant
    
    # 尽管前端tree组件提供了重复key检查功能，但是这里还是加上后端校验吧,以防未来的api请求permit
    out_list =[]
    for ns in walk_dict(out_permit):
        if ns.get('value'):
            if ns.get('value') in out_list:
                raise UserWarning('%s发生了重复'%ns.get('value'))
            out_list.append(ns.get('value'))
